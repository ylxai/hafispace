...
export function ViesusEnhancementPanel({ embedded = false }: { embedded?: boolean } = {}) {
        ...
        <div className="space-y-4">
          <div className="flex items-start justify-between gap-4">
            <div className="space-y-1">
              ...
              <p className="text-sm text-slate-600">Automatically enhance uploaded images using VIESUS technology to improve contrast, saturation, and brightness.</p>
              <p className="text-xs text-slate-500 mt-1">Free - 50 Monthly Units available for VIESUS enhancement</p>
            </div>
            <label className="relative inline-flex h-6 w-11 items-center rounded-full bg-slate-200 transition-colors focus-within:outline-none focus-within:ring-2 focus-within:ring-slate-400 focus-within:ring-offset-2">
              <input type="checkbox" className="sr-only peer" checked={enableViesus} onChange={handleSave} disabled={isSaving} />
              <span className={`absolute left-0.5 top-0.5 h-5 w-5 rounded-full bg-white shadow-lg transition-transform ${enableViesus ? "translate-x-5" : ""}`} />
            </label>
          </div>
          {enableViesus ? (
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
      ...
      <p className="mt-2 text-sm text-slate-600">Enable automatic image enhancement using VIESUS technology.</p>
      ...
      {isOpen && <div className="mt-6 border-t border-slate-100 pt-6">{content}</div>}
    </div>
  );
}
