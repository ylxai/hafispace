...
export function AccessControlPanel({ embedded = false }: { embedded?: boolean } = {}) {
    ...
    <div className="space-y-4">
      <div className="rounded-xl bg-slate-50 p-4">
        ...
        <div className="mt-3 flex items-center gap-2">
          <input readOnly value={`${baseUrl}/invite`}
            className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs text-slate-600 outline-none" />
          <button type="button" onClick={() => navigator.clipboard.writeText(`${baseUrl}/invite`)}
      ...
      <div className="rounded-xl bg-slate-50 p-4">
        ...
        <span className="mt-2 inline-block rounded-full bg-green-100 px-3 py-1 text-xs font-medium text-green-700">✓ Secured</span>
      </div>
    </div>
  );
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
      ...
      <p className="mt-2 text-sm text-slate-600">Manage admins, client invitations, and permissions.</p>
