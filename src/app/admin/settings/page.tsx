...
function CloudinaryIntegrationPanel() {
  const [isOpen, setIsOpen] = useState(false);
  const [isSaving, setIsSaving] = useState(false);
  const [saved, setSaved] = useState(false);
  const [error, setError] = useState("");
  ...
  const [config, setConfig] = useState({ cloudName: "", apiKey: "", apiSecret: "" });
  ...
  async function loadConfig() {
    ...
    try {
      ...
      if (res.ok) {
        const data = await res.json() as { cloudName?: string; apiKey?: string };
        if (data.cloudName) {
          setHasConfig(true);
          setConfig({ cloudName: data.cloudName, apiKey: data.apiKey ?? "", apiSecret: "" });
        }
      }
    } catch (err) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const form = e.currentTarget;
    const payload = {
      cloudName: (form.elements.namedItem("cloudName") as HTMLInputElement).value,
      apiKey: (form.elements.namedItem("apiKey") as HTMLInputElement).value,
      apiSecret: (form.elements.namedItem("apiSecret") as HTMLInputElement).value,
    };
    ...
    try {
      ...
      setHasConfig(true);
      setConfig({ ...payload, apiSecret: "" }); // Don't store API secret in state
      setSaved(true);
      setTimeout(() => setSaved(false), 3000);
    } catch (err: unknown) {
      setError((err as Error).message || "Failed to save Cloudinary configuration");
    } finally {
      setIsSaving(false);
    }
  }
  ...
  async function handleDisconnect() {
    if (!window.confirm("Are you sure you want to disconnect your Cloudinary account?")) {
      return;
    }
    ...
    try {
      ...
      setHasConfig(false);
      setConfig({ cloudName: "", apiKey: "", apiSecret: "" });
      setSaved(true);
      setTimeout(() => setSaved(false), 3000);
    } catch (err) {
      setError("Failed to disconnect Cloudinary account");
      console.error(err);
    }
  }
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
      <h2 className="text-lg font-semibold text-slate-900">Cloudinary Integration</h2>
      <p className="mt-2 text-sm text-slate-600">
        Connect your Cloudinary account to store and manage photos.
      </p>
      <button
        type="button"
        onClick={handleToggle}
        className="mt-4 rounded-full border border-slate-200 px-4 py-2 text-xs font-semibold uppercase tracking-[0.25em] text-slate-600 transition hover:border-slate-300 hover:text-slate-900"
      >
        {isOpen ? "Close" : hasConfig ? "Manage" : "Connect"}
      </button>
          ...
          {loading ? (
            ...
            <div className="space-y-4">
              <div className="rounded-xl bg-green-50 p-4">
                <p className="text-sm font-medium text-green-700">✓ Connected to Cloudinary</p>
                <p className="mt-1 text-xs text-green-600">Your Cloudinary account is connected and ready to use.</p>
              </div>
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div>
                  <label className="mb-1 block text-xs font-medium text-slate-600" htmlFor="cloudName">
                    Cloud Name
                  </label>
                  <input
                    id="cloudName"
                    name="cloudName"
                    type="text"
                    defaultValue={config.cloudName}
                    placeholder="your-cloud-name"
                    className="w-full rounded-xl border border-slate-200 px-4 py-2 text-sm outline-none focus:border-slate-400"
                  />
                </div>
                <div>
                  <label className="mb-1 block text-xs font-medium text-slate-600" htmlFor="apiKey">
                    API Key
                  </label>
                  <input
                    id="apiKey"
              ...
              <div>
                <label className="mb-1 block text-xs font-medium text-slate-600" htmlFor="apiSecret">
                  API Secret
                </label>
                <input
                  id="apiSecret"
              ...
              <div className="flex items-center gap-3 pt-2">
                ...
                <button
                  ...
                  disabled={isSaving}
                  className="rounded-full border border-red-200 px-5 py-2 text-sm font-medium text-red-600 transition hover:bg-red-50 disabled:opacity-50"
                >
                  Disconnect
                </button>
                {saved && (
            ...
            <div className="space-y-4">
              <div className="rounded-xl bg-slate-50 p-4">
                <p className="text-sm font-medium text-slate-700">Connect to Cloudinary</p>
                <p className="mt-1 text-xs text-slate-500">
                  Add your Cloudinary credentials to connect your account.
                </p>
              </div>
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div>
              ...
              <div className="pt-2">
                <button
                  ...
                  className="rounded-full bg-slate-900 px-5 py-2 text-sm font-medium text-white transition hover:bg-slate-700 disabled:opacity-50"
                >
                  {isSaving ? "Connecting..." : "Connect to Cloudinary"}
                </button>
                {saved && (
                  <span className="ml-3 text-sm text-green-600">✓ Connected successfully</span>
                )}
              </div>
            </div>
          )}
